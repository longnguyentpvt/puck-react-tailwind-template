...
export type BlockProps = WithLayout<{
  containerType: "full-width" | "container";
  gap: string;
  items: Slot;
}>;

const BlockInternal: ComponentConfig<BlockProps> = {
  label: "Block",
  fields: {
    containerType: {
  ...
  render: ({ containerType, gap, items: Items }) => {
    ...
    return (
      <Items
        className={`${containerClass} flex flex-col h-full ${gutterClass}`}
      />
    );
  },
};

export const Block = withLayout(BlockInternal);
